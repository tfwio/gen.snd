...
namespace AvUtil.Core
{
	...
	/// </summary>
	public class MediaFoundationReaderX : WaveStream
	{
		...
		public MediaFoundationReaderX(string file) : this(file, new MediaFoundationReaderSettings())
		{
		}
		...
		private long GetLength(IMFSourceReader reader)
		{
			...
			int hResult = reader.GetPresentationAttribute(MediaFoundationInterop.MF_SOURCE_READER_MEDIASOURCE, MediaFoundationAttributes.MF_PD_DURATION, out variant);
		...
		public override long Position {
			...
			set {
				...
				if (settings.RepositionInRead) {
					repositionTo = value;
					position = value;
					// for gui apps, make it look like we have alread processed the reposition
				}
				else {
					Reposition(value);
				}
